...
@st.cache_data
def load_data():
    local_path = BASE_DIR / "final_clustered_music_dataset.csv"
...
try:
    ...
    st.error(
        f"Could not load the local dataset at '{BASE_DIR / 'final_clustered_music_dataset.csv'}'. "
        "Please verify the file exists in the same folder as this script."
    )
    st.stop()
...
# MODE 2 - LIVE CLUSTER EXPLORER
    ...
    cluster_summaries = {
        ...
        * **Acousticness:** **Very High** *(Predominant feature)*
        * **Energy / Loudness:** **Low** *(Soft, mellow dynamics)*
        * **Instrumentalness:** **Elevated** *(Strong representation of instrumental tracks)*
        """,
        1: """
        ### **Cluster 1: Commercial Pop**
        * **Energy & Danceability:** **High Peaks** *(Optimized for clubs/radio)*
        * **Loudness:** **High Volume / Compressed**
        * **Acoustic / Instrumental:** **Near Zero** *(Synthetic or heavily produced)*
        """,
        2: """
        ### **Cluster 2: Spoken Word, Rap & Podcasts**
        * **Speechiness:** **The Absolute Highest Peak** *(Voice-centric tracks)*
        * **Liveness:** **Elevated** *(Spontaneous or live-mic feel)*
        * **Instrumentalness:** **Zero** *(No lingering arrangements)*
        """,
        3: """
        ### **Cluster 3: Melodic Grooves**
        * **Valence (Positivity):** **Highest Peak** *(Happy, upbeat tracks)*
        * **Danceability:** **Strong** *(Rhythmic and groovy rhythm sections)*
        * **Acousticness:** **Low** *(Primarily modern electronic/amplified instrumentation)*
        """,
    }
    ...
    with col_img:
        ...
        radar_path = BASE_DIR / radar_filename
        if radar_path.exists():
            st.image(radar_path, caption=f"Musical DNA Radar: Cluster {selected_cluster}", use_container_width=True)
        else:
